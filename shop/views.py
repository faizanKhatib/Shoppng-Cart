...
def index(request):
    ...
    # Display products in category-wise
...
def search(request):
    ...
    for cat in cats:
        ...
        prod = [item for item in prodtemp if searchMatch(query, item)] #this line will execute the entered product name has match or not
...
def checkout(request):
    if request.method=="POST":
        ...
        alternate_address=request.POST.get('address2', '')#+ " " + request.POST.get('address2', '')
        city=request.POST.get('city', '')
        state=request.POST.get('state', '')
        zip_code=request.POST.get('zip_code', '')
        phone=request.POST.get('phone', '')
        order = Order(items_json= items_json, name=name, amount=amount, email=email, address= address, alternate_address=alternate_address, city=city, state=state, zip_code=zip_code, phone=phone)
        order.save()
